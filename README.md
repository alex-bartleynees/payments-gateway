...
## Data migration from the monolith's `paymentsdb`

This service ships a fresh `InitialCreate` migration with `ProductId` already in the schema and targets a
new database (`PaymentsGatewayDB`). To carry over existing customers/subscriptions from the monolith's
`paymentsdb`:

1. Create `PaymentsGatewayDB` and run the gateway migrations (`dotnet ef database update`, or migrate-on-boot).
2. Copy `CustomerMappings`, `SubscriptionStates`, `InboxMessages` rows across.
3. Backfill `ProductId = 'dopamine-kick'` on every copied row.

(The old `__EFMigrationsHistory` belongs to the monolith and is **not** copied.)
